...
public static class ApprovedPlan
{
    ...
    public static byte[] Serialize(InstallationPlan plan)
    {
        ...
        var bytes = new UTF8Encoding(false).GetBytes(JsonConvert.SerializeObject(plan));
    ...
    internal static InstallationPlan Verify(byte[] bytes, string? expectedDigest)
    {
        ...
        try
        {
            var plan = JsonConvert.DeserializeObject<InstallationPlan>(new UTF8Encoding(false, true).GetString(bytes)) ?? throw Invalid("The plan is empty.");
            ExecutionPlanValidator.ValidatePlanStructure(plan);
            return plan;
        }
        catch (JsonException) { throw Invalid("The plan could not be read."); }
        catch (DecoderFallbackException) { throw Invalid("The plan could not be read."); }
    }
