...
public sealed class SourceImageLease : IDisposable
{
    ...
    public static SourceImageLease Open(InstallationPlan plan, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
                         ...
                         .Distinct(StringComparer.OrdinalIgnoreCase))
            {
                ...
                try { extents = GetExtents(file.SafeFileHandle); }
                catch (IOException) when (IsPinnedIsoSource(plan, path))
                {
                    // An ISO on a network share is not needed once its verified local extraction is applied.
                    continue;
                }
                EnsureSeparate(extents, plan.DestinationPartition, path);
            }
            if (plan.Source.ImageSha256 != null)
            {
                // The handle above blocks writers, so the verified bytes are the bytes applied.
                var image = lease._files.First(x => string.Equals(Path.GetFullPath(x.Name), Path.GetFullPath(plan.Source.ImagePath),
                    StringComparison.OrdinalIgnoreCase));
                if (!Hex.FixedTimeEquals(HashFrom(image, cancellationToken), plan.Source.ImageSha256))
                    throw new ESDInstallerException("ValidationSourceChanged",
                        "The extracted installation image changed after it was inspected. Select the image again. " + plan.Source.ImagePath);
            }
            return lease;
        }
        ...
        {
            lease.Dispose();
            if (exception is ESDInstallerException || exception is OperationCanceledException) throw;
    ...
    private static bool IsPinnedIsoSource(InstallationPlan plan, string path) =>
        plan.Source.Kind == WindowsImageKind.Iso && plan.Source.ImageSha256 != null &&
        !string.Equals(Path.GetFullPath(path), Path.GetFullPath(plan.Source.ImagePath), StringComparison.OrdinalIgnoreCase);

    internal static string HashFrom(Stream stream, CancellationToken cancellationToken)
    {
        stream.Position = 0;
    ...
    internal static IReadOnlyList<SourceExtent> GetExtents(SafeFileHandle file)
    {
        ...
        var volumeRoot = finalPath.Substring(0, end + 2);
